...
export default function ExperiencePage() {
  ...
  const canSummarize = Boolean(sessionId) && userMessageCount >= 2;
  ...
  async function generateSummary() {
    ...
    if (!response.ok) {
      setSummaryError(data.error ?? "暫時無法整理這段對話，請稍後再試。");
      return;
    }
    ...
    <main className="mx-auto max-w-4xl px-5 py-8">
          ...
          {canSummarize && (
            <div className="flex justify-center">
              <button
                ...
                className="inline-flex items-center gap-2 rounded-full border border-[#d8c8b2] bg-white px-5 py-3 text-sm font-semibold text-[#51685a] shadow-sm hover:bg-[#f8f5ee] disabled:opacity-60"
              >
                <FileText size={17} aria-hidden="true" />
                {summaryLoading ? "正在整理..." : summary ? "查看本次小結" : "整理本次對話"}
              </button>
            </div>
          )}
      ...
      {summaryOpen && summary && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-[#26332d]/30 px-4 py-4 backdrop-blur-sm sm:items-center">
          <section className="max-h-[88vh] w-full max-w-2xl overflow-y-auto rounded-[1.5rem] border border-[#e6dfd3] bg-[#fffdf7] p-5 shadow-2xl sm:p-7">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-sm font-semibold uppercase text-[#51685a]">Conversation Summary</p>
                <h2 className="mt-2 text-2xl font-semibold text-[#26332d]">{summary.title}</h2>
              </div>
              <button
                type="button"
                onClick={() => setSummaryOpen(false)}
                className="grid size-10 shrink-0 place-items-center rounded-full bg-[#f8f5ee] text-[#51685a] hover:bg-[#eef2ea]"
                aria-label="關閉小結"
              >
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row">
              <button
                type="button"
                onClick={() => setSummaryOpen(false)}
                className="inline-flex items-center justify-center rounded-lg border border-[#d8c8b2] bg-white px-5 py-3 font-semibold text-[#51685a] hover:bg-[#f8f5ee]"
              >
                回到聊天
              </button>
              <button
              ...
              <Link
                href={
                  user
                    ? `/records/${sessionId}`
                    : `/auth?mode=register&redirect=/records/${sessionId}`
                }
